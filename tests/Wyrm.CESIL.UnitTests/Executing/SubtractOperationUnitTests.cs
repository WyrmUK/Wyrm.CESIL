...
{
    public class SubtractOperationUnitTests
    {
        [Theory]
        [InlineData(10L)]
        [InlineData(StoreName)]
        public async Task ExecuteAsync_Should_Subtract_From_Accumulator(object value)
        {
            InitialiseMock();
            ...
            if (value is long longValue)
            {
                Mock.Get(_operationState)
                    .VerifySet(x => x.Accumulator = InitialAccumulatorValue - longValue, Times.Once);
            }
            else
            {
                Mock.Get(_operationState)
                    .VerifySet(x => x.Accumulator = InitialAccumulatorValue - InitialStoreValue, Times.Once);
            }
            Mock.Get(_operationState)
